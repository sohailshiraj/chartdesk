...
export function PageExceptions({ exceptions, totalPages, onChange }: PageExceptionsProps) {
  const add = () =>
    onChange([...exceptions, { page: 1, position: "top-left" }]);

  const update = (i: number, field: keyof PageException, value: number | Position) => {
    const updated = exceptions.map((e, idx) =>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-medium text-gray-700">Page Exceptions</p>
      ...
      {exceptions.length === 0 && (
        <p className="text-xs text-gray-400 italic">No exceptions — default position applies to all pages.</p>
      )}
